-- This model aggregates monthly AQI data per state, calculating average pollutant values and determining the
-- prominent pollutant and AQI category for each month.
-- It joins the fact table with dimension tables for measurement time and locations to provide a comprehensive view
-- of air quality data across different states.
...
WITH enriched_aqi_facts AS (
    SELECT
        HASH(aqi_location.STATE, aqi_time.AQI_YEAR, aqi_time.AQI_MONTH) AS RECORD_PK,
        aqi_fact.location_fk,
        aqi_location.state as STATE,
...
SELECT
   RECORD_PK,
    ...
    ROUND(AVG(NH3_AVG_VALUE)) AS NH3_AVG,
...
FROM enriched_aqi_facts
 {% if is_incremental() %}
    ...
    {% endif %}
GROUP BY RECORD_PK, YEAR, MONTH, STATE
